...
search_result_dir = '/media/F/Data/Landmarks/'
%directory where you want to download the images
output_dir = ['/media/F/Data/Landmarks/'];
    ...
    if ( ~exist([output_dir current_filename_fh], 'file'))
        ...
        while 1
          ...
          if strncmp(line,'photo:',6)
            ...
            url = ['http://farm1.staticflickr.com/' server '/' id '_' secret '.jpg'];
            ...
            if(isempty(current_file_info))
                ...
                if(current_file_size < 5000) %if the file is less than 20k, or we got an error .gif instead
                   ...
                   url = ['http://farm1.staticflickr.com/' server '/' id '_' secret '_o.jpg'];
                   cmd = ['wget -t 3 -T 5 --quiet ' url ...
                             ' -O ' '/tmp/' id '_' secret '_' server '_' owner '.jpg' ];
                ...
                if(current_file_valid == 1)
                    ...
                    if(size(current_image,3) == 3 && ... %make sure it's color
                       ...
                       min_dim_pixels >= 300 && ...
                       min_dim_pixels <= 1920)  %1920 min dimension largest allowable size.  This should almost NEVER happen, because if
                        ...
                        if(min_dim_pixels >= 300)
                            ...
                            output_filename = [output_dir current_filename_fh '/' sprintf('%.5d', dircount) '/' current_filename_fh '_' num2str(count) '_' id];

                            %lets save all the info we'll need in the comment
                            %section of the file.  We can retrieve this later
                            %with imfinfo()
                            try
                                imwrite(current_image, [output_filename '.jpg'], 'jpg', 'quality', 85, 'comment', comment_field);
            ...
            try
            %    copyfile(['/tmp/' id '_' secret '_' server '_' owner '.jpg' ], [output_dir current_filename_fh '/' sprintf('%.5d', dircount)]);
